function Mztask_build()
{
    server_name="mztask"
    server_port=7005
    echo -e  "\033[47;30m ------ Check Docker Status ----- \033[0m \n"
    docker run -d --name=$server_name -p $server_port:8080 -v /etc/localtime:/etc/localtime -v /www/servers/$server_name:/www/$server_name -v /data/logs/$server_name:/usr/local/tomcat/logs --privileged=true tomcat
    sleep 1
    ...
    if [ $(docker ps |grep $server_name |wc -l) == 1 ];then
        ...
        echo -e "\033[47;5m ------$server_name install failed !Please check !! ----- \033[0m \n"
        exit 1
    fi
}